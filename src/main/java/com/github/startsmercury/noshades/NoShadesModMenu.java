...
public class NoShadesModMenu implements ModMenuApi {
	...
	private static void save() {
		...
		if (NoShadesMod.getConfig().isReloadOnSave()) {
			NoShadesMod.LOGGER.info(() -> "Reloading chunks...");
			MinecraftClient.getInstance().worldRenderer.reload();
		}
	}

	private static void setReloadOnSave(final boolean reloadOnSave) {
		NoShadesMod.getConfig().setReloadOnSave(reloadOnSave);
	}

	private Screen genModConfigScreen(final Screen parent) {
	...
	private IntegerSliderEntry getBlockShadeMultiplier(final ConfigEntryBuilder entryBuilder) {
	...
	private IntegerSliderEntry getFluidShadeMultiplier(final ConfigEntryBuilder entryBuilder) {
	...
	private ConfigCategory getMainCategory(final ConfigBuilder builder, final ConfigEntryBuilder entryBuilder) {
	...
	private BooleanListEntry getReloadOnSaveToggle(final ConfigEntryBuilder entryBuilder) {
	...
	private SubCategoryListEntry getShadeMultiplierSubCategory(final ConfigEntryBuilder entryBuilder) {
