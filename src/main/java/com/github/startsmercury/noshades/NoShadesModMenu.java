...
public class NoShadesModMenu implements ModMenuApi {
	...
	private IntegerSliderEntry getBlockShadeMultiplier(ConfigEntryBuilder entryBuilder) {
		final IntSliderBuilder blockShadeMultiplierBuilder = entryBuilder.startIntSlider(
				...
				NoShadesMod.getBlockShadeMultiplier(), 0, 100);

		blockShadeMultiplierBuilder.setSaveConsumer(NoShadesMod::setBlockShadeMultiplier);
		blockShadeMultiplierBuilder.setDefaultValue(100);
		blockShadeMultiplierBuilder
				.setTextGetter(blockShadeMultiplier -> new TranslatableText(switch (blockShadeMultiplier) {
				case 0 -> "entry.noshades.main.shade_multiplier.block.tooltip.off";
				case 100 -> "entry.noshades.main.shade_multiplier.block.tooltip.default";
	...
	private IntegerSliderEntry getFluidShadeMultiplier(ConfigEntryBuilder entryBuilder) {
		final IntSliderBuilder fluidShadeMultiplierBuilder = entryBuilder.startIntSlider(
				...
				NoShadesMod.getFluidShadeMultiplier(), 0, 100);

		fluidShadeMultiplierBuilder.setSaveConsumer(NoShadesMod::setFluidShadeMultiplier);
		fluidShadeMultiplierBuilder.setDefaultValue(100);
		fluidShadeMultiplierBuilder
				.setTextGetter(fluidShadeMultiplier -> new TranslatableText(switch (fluidShadeMultiplier) {
				case 0 -> "entry.noshades.main.shade_multiplier.fluid.tooltip.off";
				case 100 -> "entry.noshades.main.shade_multiplier.fluid.tooltip.default";
	...
	private IntegerSliderEntry getGlobalShadeMultiplier(ConfigEntryBuilder entryBuilder) {
		final IntSliderBuilder globalShadeMultiplierBuilder = entryBuilder.startIntSlider(
				new TranslatableText("entry.noshades.main.shade_multiplier.global.title"),
				NoShadesMod.getGlobalShadeMultiplier(), 0, 100);

		globalShadeMultiplierBuilder.setSaveConsumer(NoShadesMod::setGlobalShadeMultiplier);
		globalShadeMultiplierBuilder.setDefaultValue(100);
		globalShadeMultiplierBuilder
				.setTextGetter(globalShadeMultiplier -> new TranslatableText(switch (globalShadeMultiplier) {
				case 0 -> "entry.noshades.main.shade_multiplier.global.tooltip.off";
				case 100 -> "entry.noshades.main.shade_multiplier.global.tooltip.default";
				default -> String.valueOf(globalShadeMultiplier);
				}));

		return globalShadeMultiplierBuilder.build();
	}
	...
	private SubCategoryListEntry getShadeMultiplierSubCategory(ConfigEntryBuilder entryBuilder) {
		...
		shadeMultiplierCategory.setTooltip(Optional.of(new Text[] { new TranslatableText("category.noshades.main.shade_multiplier.tooltip0"),
				new TranslatableText("category.noshades.main.shade_multiplier.tooltip1"), new TranslatableText("category.noshades.main.shade_multiplier.tooltip2"),
				new TranslatableText("category.noshades.main.shade_multiplier.tooltip3") }));
		shadeMultiplierCategory.setExpanded(true);
		shadeMultiplierCategory.add(getGlobalShadeMultiplier(entryBuilder));
